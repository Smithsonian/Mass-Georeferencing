...
\copy wikidata_names(source_id, language, name) from 'wikidata_records.csv' CSV;
\copy wikidata_descrip(source_id, language, description) from 'wikidata_records.csv' CSV;

UPDATE TABLE wikidata_records SET the_geom = ST_SETSRID(ST_POINT(longitude, latitude), 4326), gadm2 = g.name_2 || ', ' || g.name_1 || ', ' || g.name_0 FROM gadm2 g WHERE ST_INTERSECTS(g.the_geom, ST_SETSRID(ST_POINT(longitude, latitude), 4326));
